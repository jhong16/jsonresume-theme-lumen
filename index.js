...
Handlebars.registerHelper('sectionList', function(resume, sections) {
  
  const present = sections.filter(name => Array.isArray(resume[name]) && resume[name].length);
  const formatted = present.map(s => s.toUpperCase());
